...
import io
import shutil
import subprocess  # Triggers the native Mac popups
from PIL import Image
from datetime import datetime
from ollama import Client

def get_directories():
    """Summons beautiful, native macOS Finder windows using AppleScript."""
    print("Summoning native macOS folder selection...")
    
    def ask_mac_folder(prompt_text):
        # Tells macOS to open a native folder dialog
    ...
    source_dir = ask_mac_folder("Select SOURCE Folder (The Messy Files)")
    if not source_dir:
...
def analyze_with_vision(b64_images):
    ...
    try:
        ...
        # Regex armor to strip out hallucinatory text
...
def process_directory():
    # 1. Trigger the Native Mac Popups
    ...
    for filename in os.listdir(source_dir):
        # The Non-PDF Shield: ignores images, word docs, hidden files
        ...
        if ai_data and "date" in ai_data and "doc_type" in ai_data:
            ...
            # Safely moves and renames the file

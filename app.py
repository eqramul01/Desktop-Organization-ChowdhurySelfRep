...
# Worker Pool: Strictly capped to 8 Bots
...
class LegalSorterApp(ctk.CTk):
    def __init__(self):
        ...
        self.radio_deep = ctk.CTkRadioButton(self.frame_scan, text="Deep Scan (Include all subfolders)", variable=self.scan_mode_var, value=True)
        self.radio_deep.grid(row=0, column=1, padx=10, pady=10, sticky="w")
    ...
    def apply_ghost_correction_regex(self, text):
        """Phase 2 Ghost Correction: Strip AI conversational filler."""
        if not text:
            return ""

        sanitized = re.sub(
            r"^(Sure|Certainly|Here is the|I can help with|As an AI).*?[\n\r]",
            "",
            text,
            flags=re.IGNORECASE,
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_WORKERS) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    self.log_message(f"   [ERROR] Worker thread failure: {e}")
